...
if st.button("🚀 启动深度风控核查"):
   ...
   # ==========================================
    ...
    # ==========================================
    # 修复点：将下方的 {stocks} 全部改为了 {stock_name}
    prompt = f"""
    今天是 {today}。
    ...
    请严格针对以下4个维度进行事实核查，并以 JSON 格式返回：
        ...
        4. 【monitoring】(监管监控 - 严重异常波动硬指标)：
           - 搜索指令：搜索“{stock_name} 严重异常波动 重点监控 异动公告 停牌核查 10天100% 30天200% 换手率”。
           - **必须核查以下交易所硬性标准**：
             A. **严重异动（时间与空间红线）**：
        ...
        [{{"name": "{stock_name}", "reduction": "具体日期及内容...", "fraud": "造假详情...", "delisting": "退市红线详情...", "monitoring": "监控硬性指标详情..."}}]
